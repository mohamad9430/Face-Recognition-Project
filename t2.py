...
UPLOAD_FOLDER = r'C:\Users\admin\AndroidStudioProjects\face23\images'
...
def compare_images(image_name1, image_name2):
    with sqlite3.connect(db3) as conn:
        ...
        cursor.execute("SELECT image_path FROM ImageTable2 WHERE image_name = ?", (image_name1,))
        result1 = cursor.fetchone()

        cursor.execute("SELECT image_path FROM ImageTable2 WHERE image_name = ?", (image_name2,))
...
def send(path, name):
    ...
    with sqlite3.connect(db3) as conn:
        ...
        cursor.execute("INSERT INTO ImageTable2 (image_name, image_path) VALUES (?, ?)",
                       (name, path))

    print("Data inserted successfully.")


def send3(name, region):
    # print("Current Working Directory:", os.getcwd())

    with sqlite3.connect(db3) as conn:
        cursor = conn.cursor()

        cursor.execute("INSERT INTO ImageTable2 (image_name, region) VALUES (?, ?)",
                       (name, region))
...
def extract_images(image_name):
    with sqlite3.connect(db3) as conn:
        ...
        cursor.execute("SELECT image_path FROM ImageTable2 WHERE image_name = ?", (image_name,))
...
def process_images():
    ...
    with sqlite3.connect(db3) as conn:
        ...
        cursor.execute("SELECT image_name, image_path, region FROM ImageTable2 WHERE image_name != ?", (image_name1,))
        result1 = cursor.fetchall()
        print(result1)

        if result1 and extracted_face_paths:

            for image_name, image_path1, region in result1:
                print(image_name, image_path1, region)
    ...
    if file and allowed_file(file.filename):
        ...
        with sqlite3.connect(r"C:\Users\admin\Desktop\t4.db") as conn:
            ...
            try:
                ...
                cursor.execute("INSERT INTO ImageTable2 (image_path, image_name, region) VALUES (?, ?, ?)",
...
def insert_image_info(file_path, image_name, region):
    try:
        ...
        with sqlite3.connect(db3) as conn:
            ...
            cursor.execute("INSERT INTO ImageTable2 (image_path, image_name, region) VALUES (?, ?, ?)",
                           (file_path, image_name, region))
        print("Image information inserted into the database successfully.")
    except Exception as e:
        print("Error inserting image information into the database:", e)
